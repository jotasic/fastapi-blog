...
@pytest.mark.anyio
async def test_get_post_list(session: AsyncSession):
    filter_params = PostFilterParams()
    posts = await crud.get_post_list(session=session, params=filter_params)
    assert type(posts) is list


@pytest.mark.anyio
async def test_get_post_list_with_filter(session: AsyncSession):
...
@pytest.mark.anyio
async def test_get_post_by_short_id(session: AsyncSession):
...
async def test_create_post(session: AsyncSession):
    ...
    post_db = await crud.create_post(session=session, post_in=post_in)
